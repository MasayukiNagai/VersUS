...
# missense_set = set(recordsMissense['IdList'])
# print("{} mutations are found".format(recordsMissense['Count']))
# print(len(missense_set))

# detailMissesnse = Entrez.efetch(db='clinvar', id=558835, rettype='vcv', is_variationid="true", from_esearch="true")
...
def getVUS_Clinvar(idLists, genes):
    ...
    ct = 0  # debug
    i = 0
    remainder = len(idLists) % 10000
    while i < len(idLists):
        missense_info = Entrez.efetch(db='clinvar', id=idLists[i:i+10000], rettype='vcv', is_variationid="true",
                                      #  retmax=200000,
                                      from_esearch="true",
        ...
        for variation in root.findall('./VariationArchive'):
            ...
            if name in genes:
                ...
                for mutation in variation.findall('.//ProteinExpression'):
                    ...
                    before = aatranlation.get(change[0:3])  # add a condition for non-missnese
...
# df_Clinvar = getVUS_Clinvar(recordsMissense['IdList'][60000:100000], human_genes)
# print(df_Clinvar)
